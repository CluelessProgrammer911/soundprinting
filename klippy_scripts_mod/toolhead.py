...
class ToolHead:
    ...
    def drip_move_mod(self, newpos, speed, drip_completion):
        """
        Modified drip move that supports full position updates including extruder.
        Unlike drip_move, this does NOT replace extruder position with commanded_pos.
        Use for continuous motion patterns where E-axis control is needed.
        """
        # Pad newpos to match commanded_pos length if needed
        newpos = list(newpos) + list(self.commanded_pos[len(newpos):])
        # Create and verify move is valid
        move = Move(self, self.commanded_pos, newpos, speed)
        if move.move_d:
            if move.is_kinematic_move:
                self.kin.check_move(move)
            # Also check extruder move if present
            for e_index, ea in enumerate(self.extra_axes):
                if move.axes_d[e_index + 3]:
                    ea.check_move(move, e_index + 3)
        # Make sure stepper movement doesn't start before nominal start time
